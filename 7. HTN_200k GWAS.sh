...
# run with: sh 02-step1-qc-filter.sh
	...
	run_plink_qc="plink2 --bfile ukb_c1-22_GRCh38_full_analysis_set_plus_decoy_hla_merged\
	 --keep HTN_wes_200k.phe --autosome \
	 --maf 0.01 --mac 20 --geno 0.1 --hwe 1e-15 \
	 --mind 0.1 --write-snplist --write-samples \
	 --no-id-header --out HTN_WES_array_snps_qc_pass"
	...
	dx run swiss-army-knife \
	   ...
	   -iin="${data_file_dir}/file-GxQZv2jJp2pf0Y2p25J1BzG8" \
...
# run with: sh partD-step1-qc-regenie.sh
...
run_regenie_step1="regenie --step 1 --lowmem --out HTN_results --bed ukb_c1-22_GRCh38_full_analysis_set_plus_decoy_hla_merged --phenoFile HTN_wes_200k.phe --covarFile HTN_wes_200k.phe --extract HTN_WES_array_snps_qc_pass.snplist --phenoCol HTN_cc --covarCol age --covarCol sex --covarCol ethnic_group --covarCol ever_smoked --bsize 1000 --bt --loocv --gz --threads 16"
...
dx run swiss-army-knife \
	...
	-iin="${data_file_dir}/file-GvZyFG8Jp2pvKJx7ZXvzGvjZ" \
	-iin="${data_file_dir}/file-GvVgF58Jp2ppzX5VZ3jxKX1z"\
	-iin="${data_file_dir}/file-GxQb448J70qGQ8PxfpPY9vbG" \
	-iin="${data_file_dir}/file-GxQZv2jJp2pf0Y2p25J1BzG8" \
...
# Run with: sh partE-step2-qc-filter.sh  
	...
	for chr in {1..22}; do run_plink_wes="plink2 --bfile ${data_field}_c${chr}_b0_v1 --no-pheno --keep HTN_wes_200k.phe --autosome --maf 0.01 --mac 20 --geno 0.1 --hwe 1e-15 --mind 0.1 --write-snplist --write-samples --no-id-header --out HTN_WES_c${chr}_snps_qc_pass"; 
	dx run swiss-army-knife -iin="${exome_file_dir}/${data_field}_c${chr}_b0_v1.bed" -iin="${exome_file_dir}/${data_field}_c${chr}_b0_v1.bim" -iin="${exome_file_dir}/${data_field}_c${chr}_b0_v1.fam" -iin="${data_file_dir}/HTN_wes_200k.phe" -icmd="${run_plink_wes}" --tag="Step2" --instance-type "mem1_ssd1_v2_x16" --destination="${data_file_dir}" --brief --yes
	done
...
# edit with nano partF-step2-regenie.sh 
# Run with: sh partF-step2-regenie.sh 	
	...
	for chr in {1..22}; do
	  run_regenie_cmd="regenie --step 2 --bed ${data_field}_c${chr}_b0_v1 --out HTN_assoc.c${chr}\
	    --phenoFile HTN_wes_200k.phe --covarFile HTN_wes_200k.phe\
	    --bt --approx --firth-se --firth --extract HTN_WES_c${chr}_snps_qc_pass.snplist\
...
# edit with: nano partG-merge-regenie-files.sh 
...
merge_cmd='out_file="HTN_assoc.regenie.merged.txt"
...
dx run swiss-army-knife -iin="/${data_file_dir}/HTN_assoc.c1_HTN_cc.regenie.gz" \    
